...
export const getTickets = async (projectId: string) => {
  const url = `${REST_API_URL}/projects/${projectId}/tickets`;
  const res = await axios.get(url);
  return res.data as Ticket[];
};

export const createTicket = async (newTicket: Ticket) => {
  const url = `${REST_API_URL}/projects/${newTicket.projectId}/tickets`;
  const res = await axios.post(url, newTicket);
  return res.data as Ticket;
};

export const updateTicket = async (newTicket: Ticket) => {
  const url = `${REST_API_URL}/projects/${newTicket.projectId}/tickets/${newTicket.ticketId}`;
